...
| A2.2 | 只有 4 个同意范围 | 扩到 §97 的 22 个，分「必需」/「可选」两组、五个主题小组 | [新增] |
| A2.3 | 每项只有一句 description | 扩为 §98 八要素（为什么问 / 涉及信息 / 谁能看 / 是否必需 / 三种后果 / 怎么改 / 当前状态） | [新增] |
| A2.4 | **无常驻当前状态**；`<p aria-live="off">状态：{最后一次操作结果}</p>` | 改为常驻状态徽章（`[●] 当前：已同意 · 2026-07-02`），从服务器读取；操作结果只进 `role="status"` | **[名]** — `getByText(/状态：/)` 类断言与 `aria-live="off"` 元素消失 |
