...
- The page declares `lang="en"`; an unauthenticated page still needs a skip link and visible focus.
  *(Corrected 2026-08-16: this said `lang="zh-CN"`, which was right when the interface was Chinese. Every interface string has been English since D-9, so the declaration had become wrong in both the document and the code. Both now say `lang="en"`, and `apps/web/test/document-language.test.ts` holds it there — see C-10 in §8.)*
...
> **All ten are now closed** (C-1, C-2, C-3, C-5, C-6, C-7 and C-10 on 2026-08-16; C-4, C-8 and C-9 earlier). Each row keeps what it was, because the section was written to stop being a snapshot nobody revisits — a closed row still says what the defect was and, where it taught something, what.
>
> Two lessons recur across this batch and are worth stating once. **A test can pin a defect in place**: C-1's test asserted that the refusal explained itself, framed as helpfulness, so the leak had a green check sitting on top of it. And **a guard can be a false green**: C-6's first guard asserted the reporter's account sits inside the disclosure, and passed with the account put straight back on the card. Every fix below was mutation-checked by restoring the defect and watching the test go red — which is the only thing that distinguishes a guard from a comment.

| # | Location | Problem | Fix | Status (2026-08-16) |
|---|---|---|---|---|
| C-1 | `SupporterApp.tsx`, the 404 branch: "Not submitted: this needs the participant to have approved your relationship and to have consented to supporter contributions." | **Leaks the participant's relationship approval and consent status to their supporter** (protected existence is explained away) | Use the generic PROTECTED copy: `This submission could not be completed. If you are not sure what you can add to at the moment, look at your permissions.` | ✅ **FIXED 2026-08-16.** The refusal names neither prerequisite and points only at the supporter's own permissions, which are legitimately theirs to read. Two things were learned in the fixing. First, **the leak was legible over time, not just once**: a supporter watching the sentence stop appearing learns the moment consent was granted, and one watching it start learns the moment it was withdrawn — a single disclosure would have been the lesser version. Second, **the test was pinning the defect in place**: it asserted the refusal *did* name the prerequisites, framed as being helpful. Its replacement asserts the absence of the phrases plus one positive check that the refusal still goes somewhere, and it was mutation-checked by restoring the old string |
| C-2 | `StaffSafetyTriagePanel.tsx`: the disposition is a `<select>` whose first and therefore default option is "Close as not a safety event" | **A default value amounts to a triage conclusion nobody chose**, breaking both "nothing pre-selected" and "closing and converting carry equal weight" | Replace with a `<fieldset>` radio group with nothing pre-selected, four options (close / keep watching / escalate / convert to an event) | ✅ **FIXED 2026-08-16** — with one deliberate departure. The `<select>` is a `<fieldset>` radio group, the state starts at `null` rather than a default, and submission is blocked until a person chooses; the old test relied on the default and never chose at all, which is exactly the path a distracted reviewer would take, so it now asserts nothing is checked on render. Closing is no longer listed first, and the confirmation for it says that closing does not mean the person is safe. **The fourth disposition was not built.** `safety.safety_signals.signal_state`'s CHECK admits Recorded, Awaiting Triage, In Review, Escalated, Converted and Closed, and nothing for "keep watching" — a button for it would close the reviewer's screen, report success and record nothing, which is the empty control this project keeps dismantling (D-2/D-5/D-21). It is named on screen as absent, with what to do instead: escalate and say so in the reason, do not close |
| C-3 | `StaffSafetyTriagePanel.tsx`: the confirm button is named `Confirm` | Breaks "a confirm button must name the action" | Rename to `Confirm and record the disposition` | ✅ **FIXED 2026-08-16** — `Confirm and record the disposition`, with the cancel reading `Back, do not record it`. Read aloud out of context, `Confirm` names nothing, and this is the screen where what is being confirmed matters most |
...
| C-5 | `StaffModeratorPanel.tsx`: the confirmation says only that it cannot be changed | **No appeal route and no reversibility line** (a non-negotiable constraint) | Adopt the seven-line confirmation body from §E4, including the "appeal" and "reversibility" lines | ✅ **FIXED 2026-08-16.** Immutability is no longer the last thing said: two true things follow it — the record of the decision is permanent but its **effect** is not (a hide can be followed by a restore, recorded the same way), and the person it is about can appeal, handled by somebody other than the decider. The reasoning worth keeping is who the missing lines were failing: a moderator hesitating over a borderline case, told the decision is permanent and shown no way back, finds doing nothing the safest-feeling move — and an untouched queue is its own harm |
| C-6 | `StaffModeratorPanel.tsx`: the queue card renders `reportDescription` directly (the reporter's own account) | **The reporter's own account may contain identifying information about them, and it appears at the queue level** | Show only `reportCategory` at the queue level; move the account itself into E3's collapsed-by-default area | ✅ **FIXED 2026-08-16.** The card carries the category; the reporter's own account moved into a `<details>` closed by default, so reading it is a deliberate press and a moderator triaging by category never opens it. It was not removed — a moderator needs it to decide. **The guard for this was a false green on the first attempt**, and that is worth recording: it asserted the account sits inside the disclosure, and it passed with the account put straight back on the card, because on the card the text is interpolated into a sentence so `getByText` never matched it there. It now clones the case card, removes the disclosure, and asserts the account is absent from what remains — phrased as absence from the card rather than presence in the disclosure, and mutation-checked |
| C-7 | `SupporterApp.tsx`: the status label puts "not their own testimony" inside a parenthesis | A parenthesis is a typographic signal that can be skipped over | Make it a field of its own, `Is this the person's testimony: no`, on every card | ✅ **FIXED 2026-08-16.** `Accepted` reads plainly, and the authorship fact is its own field, `Is this the person's own testimony: no — it stays recorded as your contribution`, shown on every card in **every** state rather than only on `Accepted`: it is equally true before a decision as after one, and showing it only on acceptance would imply the question had been open beforehand |
...
| C-10 | `apps/web/index.html` declared `lang="zh-CN"` | **The document language was wrong.** Every interface string has been English since D-9, so a screen reader applied Chinese pronunciation rules to English text, and assistive technology and translation tools were told the page is in a language it is not. WCAG 3.1.1 (Language of Page) is a Level A criterion | Change to `lang="en"` | ✅ **FIXED 2026-08-16.** The declaration is now `lang="en"`, with `apps/web/test/document-language.test.ts` holding it there — anchored to the interface's actual language rather than a hard-coded `en`, so a future change of language fails here instead of drifting silently. What made this survivable is worth recording: **nothing looked wrong**. Every visual check passed and every query-by-name test passed, because the only reader affected was the one nobody was watching — which is why the guard is mechanical rather than a note |
