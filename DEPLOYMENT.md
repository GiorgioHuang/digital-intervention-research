...
4. **对象存储供应商已定为 Cloudflare R2（ADR-106），但当前部署尚未接通**——`R2_ACCOUNT_ID`/`R2_BUCKET` 变量与 `HADI_R2_ACCESS_KEY_ID`/`HADI_R2_SECRET_ACCESS_KEY` 秘密未设，所以字节仍写在 Postgres 的 `simulated_blobs` 列里。四项全部就位后下次部署即切换（部署日志会写明用的是哪一个）；**半配置不会回退，会拒绝启动**。**上传扫描仍是模拟器**（只认得 EICAR 测试串，ADR-126），界面不得出现「已查毒」。通信供应商为确定性模拟器。
