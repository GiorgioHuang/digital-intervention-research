...
- External dependencies (checked 2026-08-07, object storage rechecked 2026-08-16): the **Knowledge Platform is a real integration** (ADR-110 — a live MCP endpoint, not a simulator); **object storage is Cloudflare R2 (ADR-106) and is connected** — all four settings are present and the running revision has reported `fileStorage: object-store` at `/ready` since 2026-08-08, and **the credentials were exercised for the first time on 2026-08-22** (written, read back, deleted and confirmed gone through the platform’s own blob store); what remains unverified is the participant-facing chain — no byte has entered the bucket from a browser session; a deployment with none of the four settings falls back to a Postgres simulator, and a half-configuration refuses to start. **Still deterministic simulators** are the communications provider (HMAC callbacks + replay protection), the model/tool gateway and the malware scanner (ADR-126). **Identity is not among them** (corrected 2026-08-16): ADR-104 was ruled as Sign in with Google, the deployed revision runs `AUTH_MODE=google`, and the dev-header stub — where identity is whatever a header claims — is off there. CI runs the whole deterministic suite on every push.
- Governance artefacts: traceability.yaml (**155 entries**, validated by a tool), IMPLEMENTATION_BASELINE, PILOT_READINESS_REPORT, SECURITY_AND_PRIVACY_PLAN, THREAT_MODEL, ACCESSIBILITY_TEST_PLAN, SYNTHETIC_PILOT_PLAN.

> **Counts corrected on 2026-08-16, and the test total restored on the same day.** This section previously recorded 16 module packages, 26 migrations, 124 traceability entries and "576 deterministic tests"; the first three are counted from the tree as above.
>
> The test total had been *removed* rather than replaced, on the reasoning that only the database-free suites could be run here (kernel 50, policy 30, web 390) and that a static count of declarations — 771 — is not a verified total, so a stale exact number would read as evidence it was not. That reasoning was right about the danger and wrong about the availability: a local PostgreSQL 16 can be started here, and with `pnpm migrate:reset && pnpm migrate` against a fresh database the whole suite runs. **828 tests pass across 26 packages, 86 test files, with 0 failures** (2026-08-16; 21 further tests are skipped by their own conditions). Any exact figure here is a snapshot and goes stale the moment a test is added — it went from 824 to 828 inside the commit that first recorded it — so the number is given with the command that reproduces it rather than on its own authority. The three sub-counts above are unchanged and are the same run.
>
> One caveat is kept because it cost real time: the suite reports `exit 0` with the database unreachable, since database tests skip rather than fail. A green run proves nothing on its own — the total is what proves it, and 562 rather than 824 is what a dead Postgres looks like.
