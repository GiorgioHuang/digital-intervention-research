...
| `apps/web`（参与者工作区：任务式首页/细粒度同意/消息确认/屏蔽与报告/安全担忧/可选匹配，会话/联系/推荐均来自 API 查询；员工工作区：入组协调/研究者/批准/安全 triage，待办队列驱动、MFA 分级如实标注、决定署名确认；HTTP-only 边界） | Implemented（24 组件测试） |
