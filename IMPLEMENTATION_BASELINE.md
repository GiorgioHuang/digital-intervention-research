...
| `apps/api`（Doc 15 错误信封/上下文头/dev-header 认证桩 + 参与者侧命令端点（含 M17 Life Story 全链）+ 员工侧命令端点（M04 协议链/M05 入组链/M06 干预组合/M09 安全 triage/M12 数据集血缘/M13 分析链）+ M18 属主只读查询 + OpenAPI） | Implemented（17 e2e） |
