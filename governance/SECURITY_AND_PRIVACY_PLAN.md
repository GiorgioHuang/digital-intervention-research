...
| Encryption in transit | Implemented — HTTPS end to end (Cloud Run, Neon, R2) | the deployed revision answers over HTTPS; the smoke step calls it that way |
| Encryption at rest | **Covered by the storage vendors' own properties; not configured and not verified here** (owner's disposition, 2026-08-22). The assurance is real and it is **inherited**: nothing in this project sets an encryption option, and nothing here would fail if the property stopped holding | — none, by construction: a test would be asserting a vendor's documentation rather than this platform's behaviour |
| Key management | **Pending External Approval** (ADR-103/119/121). A rotation **procedure** now exists for all five secrets (DEPLOYMENT.md), but there is no schedule, no expiry, and no automatic check that a rotated credential works — the R2 round-trip is run by hand | DEPLOYMENT.md "Rotating a secret"; `tools/r2-round-trip.mjs` |
...
2. Hosting platform approved (ADR-103) → **key management** and network boundaries land. Three parts of this arrived without the approval rather than through it, and the distinction is the point of keeping the item open: transit is HTTPS end to end, secret management is Secret Manager with keyless Workload Identity, and encryption at rest is a property of the storage vendors (disposed 2026-08-22). None of the three was approved; the first two are how the prototype environment happens to have been built, and the third is somebody else's default. What the approval would add is not the controls but the **accountability** for them — a contract that says who is answerable when one stops holding.
