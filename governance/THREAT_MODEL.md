...
> Status as of 2026-08-21. Method: enumerate threats by trust boundary (STRIDE categories), set against the **mitigations actually implemented** (with code/test evidence) and the **residual risk**. The honesty principle: where there is no mitigation, say so; a simulator is not real protection.
...
| A rotated object-store credential failing silently (T/D) | **Not mitigated.** `/ready` derives `fileStorage` from the blob store's *description string*, which is built from configuration and never calls R2 — so a deployment carrying a wrong, expired or revoked R2 credential reports `object-store` and passes every smoke check. The deploy goes green and **the first person to find out is the first participant to upload a photograph** | `health.controller.ts` builds the value from `blobs.description`; no test asserts a live round-trip because none is made | **Open, and sharpened by the rotation procedure**: rotation is exactly when a credential is most likely to be wrong, and it is the one operation this check cannot see. Closing it means exercising the credential — a scratch round-trip during deploy, or a deep-check separate from readiness (readiness must not depend on an external service, or an R2 blip would have Cloud Run killing healthy instances) |
