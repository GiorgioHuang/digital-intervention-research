...
2. **Identity is real; the access token is not identity** (corrected 2026-08-16 — this item still said identity was a stub eight days after it stopped being one, and the banner at the top of this file had already been corrected, which is how it survived: the same claim in different words). The deployment runs `AUTH_MODE=google`, so people are distinguished and a session can be revoked for one person. The **access token** is a separate, shared-secret boundary layered in front: it does not distinguish between people and cannot be revoked for one, which is why it is a compensating control and never a description of who someone is. Real *participants* remain conditional on production-readiness approval (see PILOT_READINESS_REPORT) — that gate is about ethics and contracts, not about authentication.
