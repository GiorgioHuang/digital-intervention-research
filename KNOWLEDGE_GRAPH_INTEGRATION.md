...
- **限制**：语料为 60 节点/54 声明/25 证据的策展种子集——覆盖了优先主题域的骨架（孤独感、社会连接、怀旧/生命故事、数字干预、知识缺口），但**远不足以支撑真实证据综述**；部分节点类型缺失（无 `problem`/`model`/`risk` 独立类型）；图谱级版本号未经 API 暴露（`seed/graph.json` 内 `"version": 6` 字段存在但 /health 与 MCP 不返回）。
